...
def handle_sms_spammer(bot, message, conversation_states):
    ...
    if re.match(pattern, input_text):
        ...
        spam_handler(input_text)
        keyboard = InlineKeyboardMarkup(row_width=1)
        stop_spam_button = InlineKeyboardButton(
        text="Остановить спаммер", callback_data="stop_spam")
        keyboard.add(stop_spam_button)

        message_options = {
            "reply_markup": keyboard,
        }

        bot.send_message(chat_id, f"[✅] Spammer запущен!\n\n Телефон: {input_text}\n\n Чтобы остановить спам, нажмите кнопку ниже:", **message_options)
    else:
       bot.send_message(chat_id, "[❌] Номер телефона введён неверно!")

    conversation_states[chat_id]["sms_spammer_input"] = None


def spam_handler(phone):
    ...
    threads = 2 #!Введите кол-во потоков
    if threads in ["", " ", "0"]:
        threads = "1"
    try:
        if int(threads) > 10:
